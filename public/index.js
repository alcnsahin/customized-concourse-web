    ...
    $("g.node.job").hover(
      function(){//mouseover
        ...
        if(href !== undefined){
          ...
          var build_url = "/api/v1/teams/"+hrefArr[2]+"/pipelines/"+hrefArr[4]+"/jobs/"+hrefArr[6]+"/builds/"+hrefArr[8];
          var build_id = getJson(build_url,"id");
          var ref_url = "/api/v1/builds/"+build_id+"/resources";
          var ref = getJson(ref_url,"ref");
          //tooltip style
          $("body").append("<div id='tooltip'>REF: "+ref+"</div>");
          $("div#tooltip").css({
            "position":"absolute",
            "top":"80px",
            "right":"10px",
      ...
      function(){$("div#tooltip").remove();}//mouseout
  );//--
    ...
    success:function(data){
      if (keyName == "ref") {
        ...
        $.each(data.inputs, function(key,val){
            console.log("NAME=> "+data.inputs[i].name);
            if (data.inputs[i].version["ref"] != null) {
              result = data.inputs[i].version["ref"];
            }
            i++;
        });
